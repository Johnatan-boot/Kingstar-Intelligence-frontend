...
function SnapshotButton({ tipo, label }: { tipo: 'ALMOCO' | 'FECHAMENTO'; label: string }) {
  const [status, setStatus] = useState<'idle' | 'saving' | 'done' | 'error'>('idle');

  const handleClick = async () => {
    setStatus('saving');
    try {
      await analyticsApi.registrarSnapshot(tipo);
      setStatus('done');
  ...
  return (
    <button
      ...
      style={{
        ...
        border: '1px solid #242424', background: '#1a1a1a', color: '#c0cad6',
        cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
        gap: 6, flex: '1 1 auto', minWidth: 150, whiteSpace: 'nowrap',
      }}
    >
      {status === 'saving' ? 'Registrando...' : status === 'done' ? '✓ Registrado' : status === 'error' ? 'Erro ao registrar' : label}
    </button>
  );
}
...
export function AnalyticsMfe() {
  ...
  if (loading) return (
    <div style={{ padding: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, color: '#8b9dc3', gap: 10 }}>
      <Loader2 size={22} style={{ animation: 'spin 1s linear infinite' }} /> Carregando análises...
    </div>
  );
  ...
  const scoreColor =
    score?.classification === 'Excelente' ? '#22c55e' :
    score?.classification === 'Bom' ? '#38bdf8' :
    score?.classification === 'Regular' ? '#f59e0b' :
    score?.classification === 'Aguardando atividade' ? '#8b9dc3' :
    '#ef4444';

  return (
    <div style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      {/* Cabeçalho — quebra em coluna em telas pequenas para não estourar a largura */}
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
          <div style={{ width: 36, height: 36, minWidth: 36, borderRadius: 10, background: 'rgba(139,92,246,.12)', border: '1px solid rgba(139,92,246,.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <BarChart3 size={18} color="#8b5cf6" />
          </div>
          <div style={{ minWidth: 0 }}>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: '#fff' }}>Análise & Desempenho</h1>
            <p style={{ fontSize: 12, color: '#8b9dc3', marginTop: 2 }}>Métricas operacionais e inteligência de dados</p>
          </div>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginLeft: 'auto', width: 'auto' }}>
          <SnapshotButton tipo="ALMOCO" label="Registrar corte do almoço" />
          <SnapshotButton tipo="FECHAMENTO" label="Registrar fechamento do dia" />
        </div>
      </div>

      {/* KPI Cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(145px, 100%), 1fr))', gap: 12 }}>
        {[
          { label: 'NFs Concluídas (hoje)', value: metrics?.totalCompletedNFs ?? 0, icon: CheckCircle, color: '#22c55e' },
          { label: 'Veículos Recebidos (hoje)', value: metrics?.totalVehiclesReceived ?? 0, icon: Truck, color: '#38bdf8' },
          { label: 'Peças Conferidas (hoje)', value: metrics?.totalPiecesChecked ?? 0, icon: Package, color: '#8b5cf6' },
          { label: 'Taxa de Avarias', value: `${metrics?.errorRate ?? 0}%`, icon: AlertTriangle, color: '#ef4444' },
          ...
          { label: 'Tempo Rec. Médio', value: metrics?.avgReceivingMin ? `${metrics.avgReceivingMin} min` : '0 min', icon: Clock, color: '#06b6d4' },
          { label: 'Tempo Rec. Total', value: metrics?.totalReceivingMin ? `${metrics.totalReceivingMin} min` : '0 min', icon: Clock, color: '#22d3ee' },
          { label: 'Tempo Conf. Médio', value: metrics?.avgConferenceMin ? `${metrics.avgConferenceMin} min` : '0 min', icon: Clock, color: '#a78bfa' },
          { label: 'Tempo Conf. Total', value: metrics?.totalConferenceMin ? `${metrics.totalConferenceMin} min` : '0 min', icon: Clock, color: '#c084fc' },
          { label: 'Pedidos a Receber', value: kpis?.a_receber ?? 0, icon: Package, color: '#facc15' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
        <Card>
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: 12, marginBottom: 20 }}>
              <span style={{ fontSize: 'clamp(36px, 8vw, 52px)', fontWeight: 900, fontFamily: 'monospace', color: scoreColor }}>{score?.total ?? '—'}</span>
              <span style={{ fontSize: 'clamp(14px, 3vw, 20px)', fontWeight: 600, color: scoreColor }}>{score?.classification}</span>
            </div>
            ...
            <p style={{ fontSize: 11, color: '#8b9dc3' }}>
              Aprovação de conferências (40%) + índice de avarias (25%) + prazo de recebimento (20%) + pedidos concluídos (15%)
            </p>
          </div>
        </Card>
        ...
        <Card>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #242424' }}>
            <h3 style={{ fontWeight: 600, fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
              <TrendingUp size={16} color="#38bdf8" /> Volume de Conferências — Últimos 14 dias
            </h3>
            <p style={{ fontSize: 11, color: '#8b9dc3', marginTop: 4 }}>
              Mínimo (registrado no almoço) e máximo (registrado no fechamento) de pedidos conferidos
            </p>
          </div>
          <div style={{ padding: '20px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #242424', background: '#121212' }}>
                  {['#', 'Fornecedor', 'Pontuação', 'Entregas', 'Divergências', 'Tempo Médio'].map(h => (
