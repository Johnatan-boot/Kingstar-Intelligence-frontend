...
export function AydaCoreMfe() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', text: res.text, provider: res.provider }]);
    } catch (err) {
      console.error(err);
      setMessages(prev => [...prev, { role: 'assistant', text: 'ERRO Crítico: Fallback final falhou. Verifique integrações de endpoint logístico.', provider: 'ERROR' }]);
    } finally {
      setIsTyping(false);
    }
  };
